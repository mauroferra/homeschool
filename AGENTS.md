...
- **Schema is managed by umzug migrations**, not `db.sync()`. `npm run db:migrate` = `node src/db/migrate.js`, which runs `umzug` with `SequelizeStorage` against `src/db/migrations/*.js` (tracked in a `SequelizeMeta` table).
- **To change the schema, add a new numbered migration file** (e.g. `006_add_foo.js`) exporting `up`/`down` that use the passed `context` (a Sequelize QueryInterface) — e.g. `await context.addColumn(...)`. The initial five migrations create tables and back-port the translation columns; baseline tables check `tableExists()` so they are safe on both fresh and existing DBs. `db.sync()` is only used by the test suite (`sync({ force: true })` on its in-memory DB).
