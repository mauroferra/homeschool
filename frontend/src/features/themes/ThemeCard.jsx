...
export default function ThemeCard({ theme, onEdit, onDelete }) {
  ...
  const name = L(theme, 'name');
  const description = L(theme, 'description');
  ...
  return (
    <Card className="theme-card">
      <div className="theme-card-body">
        ...
        {description && <p className="theme-card-desc">{description}</p>}
      </div>
      <div className="theme-card-actions">
        <button type="button" className="btn-icon" onClick={() => onEdit(theme)} aria-label={t('themeCard.edit', { name })}>
          <Icon name="edit" size={18} />
        </button>
        <button type="button" className="btn-icon btn-danger" onClick={() => onDelete(theme)} aria-label={t('themeCard.delete', { name })}>
